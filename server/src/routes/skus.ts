...
import { skus, eq, ilike, or, asc, desc, and, ne, sql } from '@postgress/shared';
 ...
 */
async function generateUniqueSlug(scopedDb: any, name: string, customerId: number, excludeSkuId?: number): Promise<string> {
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    const { name, productId, vendorId, brandId, vendorSku, purchaseUrl, price, isPurchasable, currency, lifespanYears, notes, status, kind, tags } = req.body || {};
    ...
    const newSkus = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb) => {
      ...
      const slug = await generateUniqueSlug(scopedDb, name, scope.customerId);
      return scopedDb
        .insert(skus)
        .values({
          name,
...
router.post('/composite', authenticateToken, async (req, res) => {
  try {
    ...
    const created = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb, client) => {
      // Create SKU first
      const slug = await generateUniqueSlug(scopedDb, skuInput.name, scope.customerId);
  ...
  try {
    ...
    const updatedSkus = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb) => {
      if (name !== undefined) {
        ...
        updateData.slug = await generateUniqueSlug(scopedDb, name, scope.customerId, parseInt(id));
      }
      return scopedDb
        .update(skus)
        .set(updateData)
    ...
    const updated = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb, client) => {
      ...
      if (skuInput && Object.keys(skuInput).length > 0) {
        ...
        if (skuInput.name !== undefined) {
          ...
          updateData.slug = await generateUniqueSlug(scopedDb, skuInput.name, scope.customerId, parseInt(id));
        }
        ...
        await scopedDb.update(skus).set(updateData).where(eq(skus.id, parseInt(id))).returning();
      }

      // Replace components: delete existing and insert new
      if (skuComponents) {
        // delete existing
        await scopedDb.delete(skuComponents).where(eq((skuComponents as any).parentSkuId, parseInt(id)));
        if (Array.isArray(components) && components.length > 0) {
          const rows = components.map((c: any, idx: number) => ({
            parentSkuId: parseInt(id),
            componentSkuId: parseInt(c.componentSkuId),
            quantity: Number(c.quantity) || 1,
            isRequired: c.isRequired !== undefined ? !!c.isRequired : true,
            sortOrder: c.sortOrder !== undefined ? Number(c.sortOrder) : idx,
        ...
        // Raw SQL fallback
        await client.query('DELETE FROM sku_components WHERE parent_sku_id = $1', [parseInt(id)]);
        for (let i = 0; i < components.length; i++) {
          const c = components[i];
          await client.query(
            'INSERT INTO sku_components (parent_sku_id, component_sku_id, quantity, is_required, sort_order, cost_allocation) VALUES ($1, $2, $3, $4, $5, $6)',
            [parseInt(id), parseInt(c.componentSkuId), Number(c.quantity) || 1, c.isRequired !== undefined ? !!c.isRequired : true, c.sortOrder !== undefined ? Number(c.sortOrder) : i, c.costAllocation !== undefined ? Number(c.costAllocation) : null]
          );
        }
      }

      // Return the updated SKU
      const rows = await scopedDb.select().from(skus).where(eq(skus.id, parseInt(id))).limit(1);
      return rows[0];
    });
