...
import {
  bookingReservations as reservations,
  eq,
  ilike,
  and,
  or,
...
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    if (status) {
      clauses.push(eq(reservations.status, String(status).trim()));
    }

    if (homeId) {
      const home = Number(homeId);
      if (Number.isFinite(home)) {
        clauses.push(eq(reservations.homeId, home));
      }
    }

    const fromDate = parseOptionalDate(from, 'from');
    if (fromDate) {
      clauses.push(gte(reservations.checkIn, fromDate));
    }

    const toDate = parseOptionalDate(to, 'to');
    if (toDate) {
      clauses.push(lte(reservations.checkOut, toDate));
    }
    ...
    if (searchTerm) {
      ...
      clauses.push(
        or(
          ilike(reservations.confirmationCode, like),
          ilike(reservations.externalId, like),
    ...
    const rows = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb) => {
      let query = scopedDb.select().from(reservations);
      if (clauses.length === 1) {
        query = query.where(clauses[0]);
      } else if (clauses.length > 1) {
        query = query.where(and(...clauses));
      }
      return query.orderBy(desc(reservations.createdAt)).limit(limit).offset(offset);
    });
...
router.get('/:id', optionalAuth, async (req, res) => {
  try {
    ...
    const rows = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb) => {
      return scopedDb.select().from(reservations).where(eq(reservations.id, reservationId)).limit(1);
    });
    ...
    console.error('reservations detail error:', error);
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    eventBus.broadcast({
      event: 'data_change:reservations',
      data: { type: 'create', resource: 'reservations', resourceId: created.id, data: created },
...
router.put('/:id', authenticateToken, requireWriteMiddleware, async (req, res) => {
  try {
    ...
    const rows = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb) => {
      return scopedDb.update(reservations).set(updates).where(eq(reservations.id, reservationId)).returning();
    });
    ...
    eventBus.broadcast({
      event: 'data_change:reservations',
      data: { type: 'update', resource: 'reservations', resourceId: updated.id, data: updated },
      meta: { timestamp: Date.now(), source: 'api', audience: { customerId: scope.customerId, homeIds: scope.homeIds } },
    });
...
router.delete('/:id', authenticateToken, requireWriteMiddleware, async (req, res) => {
  try {
    ...
    const rows = await withTenantScope({ customerId: scope.customerId, homeIds: scope.homeIds }, async (scopedDb) => {
      return scopedDb.delete(reservations).where(eq(reservations.id, reservationId)).returning();
    });
    ...
    eventBus.broadcast({
      event: 'data_change:reservations',
      data: { type: 'delete', resource: 'reservations', resourceId: deleted.id, data: deleted },
      meta: { timestamp: Date.now(), source: 'api', audience: { customerId: scope.customerId, homeIds: scope.homeIds } },
    });
