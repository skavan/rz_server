...
- Next.js client calls the Express API in `server_v2`.
...
- We are committing `server_v2/.env` and `client/.env.local` to git to avoid losing them during active development.
- This is enabled by two negation rules in the repo root `.gitignore`:
  - `!server_v2/.env`
  - `!client/.env.local`
- When you’re ready to stop committing these files:
  1) Copy values into `server_v2/.env.example` and `client/.env.local.example`.
...
File: `server_v2/scripts/drizzle/drizzle-rebuild.ts`
...
- When you want Drizzle to “own” migrations again:
  ...
  2) Run the migrator so Drizzle creates the meta table and applies migrations: `npm run db:migrate` from `server_v2`.
...
- GET http://localhost:5000/api/table/locations
...
From `server_v2`, you can execute a .sql file safely in a single transaction:
...
- Path gotchas (Windows)
  - Prefer running from `server_v2` folder
...
You’re good to go. Keep this doc in `server_v2/docs/operational-playbook.md`. 🚀
